...
//        "M10,10 L50,60 A10,20 45 0,1 80,70 Q100,90 120,100 T140,110 L160,120 A5,10 30 0,0 180,130 Q200,150 220,160 T240,170 L260,180 A15,25 60 1,1 280,190 Q300,210 320,220 T340,230 Z"
    ...
    val animatedPath = remember(key1 = canvasSize) {
        AnimatedPath(
            arrowForwardString,
            recommend,
            width = if (canvasSize != IntSize.Zero) canvasSize.width.toFloat() else 450f,
            height = if (canvasSize != IntSize.Zero) canvasSize.height.toFloat() else 450f
        )
    }
...
//                        val startPath = PathParser().addPathNodes(rawStartPathNodes).toPath()
//                        drawPath(startPath, color = Color.Green, style = Stroke(width = 20f))
////
//                        val endPath = PathParser().addPathNodes(rawEndPathNodes).toPath()
//                        drawPath(endPath, color = Color.Red, style = Stroke(width = 20f))

//                        val pathToDraw = PathParser().addPathNodes(
//                            startPathSegments.take(sliderPosition.toInt()+1)
//                                .map { segment -> segment.pathNode }).toPath()
//                        drawPath(pathToDraw, color = Color.Black, style = Stroke(width = 40f))

                        val animationProgressValue = animationProgress.value
                        val interpolatedPath =
                            animatedPath.getInterpolatedPath(animationProgressValue)

                        val unpairedStartPathValue = if (animationProgressValue < 0.5f) {
                            animationProgressValue / 0.5f
                        } else {
                            1f
                        }
                        val unpairedStartPath = animatedPath.getUnpairedStartPath(unpairedStartPathValue)

                        val unpairedEndPathValue = if (animationProgressValue <= 0.5f) {
                            1f
                        } else {
                            (1f - ((animationProgressValue - 0.5f) / 0.5f))
                        }

                        val unpairedEndPath = animatedPath.getUnpairedEndPath(unpairedEndPathValue)
//                        val interpolatedPath =
//                            animatedPath.getInterpolatedPath(sliderPosition / (startPathSegments.size-1))
//                        drawPath(
//                            interpolatedPath,
//                            color = Color.Black,
...
//                        )
                        ...
                        drawPath(
                            ...
                            color = Color.Black,
                            style = Stroke(width = 50f, join = StrokeJoin.Round),
                            alpha = minOf(maxOf(1f - animationProgressValue, 0f), 1f)
                        )
                        drawPath(
                            unpairedEndPath,
                            color = Color.Black,
                            style = Stroke(width = 50f, join = StrokeJoin.Round),
                            alpha = minOf(maxOf(animationProgressValue, 0f), 1f)
                        )
                    }
